1.
SELECT id,名称,職業コード,hp,mp,状態コードFROM パーティー;
...
15
UPDATE パーティー SET 状態コード=(SELECT コード値 FROM コード WHERE コード名称='眠り')
WHERE 名称 LIKE '%ミ%';
...
20
SELECT * FROM パーティー
WHERE id LIKE 'A%' AND 職業コー LIKE '2%';
...
36
SELECT p.名称 AS なまえ,
concat(p.hp,'/',p.mp) As HPとMP,
CASE
WHEN c.コード値='00' OR c.コード値 IS NULL THEN '' ELSE c.コード名称
END AS ステータス
FROM パーティー AS p
